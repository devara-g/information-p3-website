...
(function () {
    ...
    const playlist = [
        {
            title: 'Hymne Guru',
            artist: 'Lagu Nasional Indonesia',
            src: 'music/Hymne Guru - Lirik Lagu Nasional Indonesia.mp3'
        },
        {
            title: 'Indonesia Raya',
            artist: 'Lagu Kebangsaan Indonesia',
            src: 'music/Indonesian National Anthem - Indonesia Raya (IDEN).mp3'
        },
        {
            title: 'Satu Nusa Satu Bangsa',
            artist: 'Lagu Nasional Indonesia',
            src: 'music/Lirik Lagu Satu Nusa Satu Bangsa  vocal by Ceo Jati Atmodjo.mp3'
        },
        {
            title: 'Tanah Airku',
            artist: 'Lagu Nasional Indonesia',
            src: 'music/TANAH AIRKU  TEKS ( 2023 ).mp3'
        },
        {
            title: 'Garuda Pancasila',
            artist: 'Lagu Nasional Indonesia',
            src: 'music/Garuda Pancasila - Lagu Nasional Indonesia (dengan Lirik).mp3'
        },
        {
            title: 'Mars PGRI',
            artist: 'Mars PGRI',
            src: 'music/MARS PGRI (lirik).mp3'
        }
    ];
    ...
    function updateProgressStyle() {
        ...
        progressBar.style.background =
            `linear-gradient(to right, rgba(255,255,255,0.85) 0%, rgba(255,255,255,0.85) ${pct}, rgba(255,255,255,0.15) ${pct}, rgba(255,255,255,0.15) 100%)`;
    }

    function updateVolumeStyle() {
        const pct = (volumeSlider.value * 100) + '%';
        volumeSlider.style.background =
            `linear-gradient(to right, rgba(255,255,255,0.75) 0%, rgba(255,255,255,0.75) ${pct}, rgba(255,255,255,0.15) ${pct}, rgba(255,255,255,0.15) 100%)`;
    }
    ...
    function loadSong(index, andPlay) {
        ...
        if (song.title.length > 22) {
            ...
            void songTitle.offsetWidth; // reflow
        ...
        document.querySelectorAll('.mp-playlist-item').forEach((item, i) => {
            ...
            icon.className = (i === index) ? 'fas fa-music' : 'fas fa-circle';
        });
    ...
    playlist.forEach((song, i) => {
        ...
        item.innerHTML = `
            <span class="mp-playlist-num">${i + 1}</span>
            <i class="${i === 0 ? 'fas fa-music' : 'fas fa-circle'}"></i>
            <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${song.title}</span>
        `;
    ...
    playlistToggle.addEventListener('click', () => {
        ...
        playlistChevron.style.transform = playlistEl.classList.contains('open')
