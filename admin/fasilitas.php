...
<div style="overflow-x:auto"><table id="fasTable" style="width:100%;border-collapse:collapse">
<thead><tr>
<th style="padding:12px 15px;width:45px">No</th>
<th style="padding:12px 15px;width:65px">Gambar</th>
<th style="padding:12px 15px">Nama</th>
<th style="padding:12px 15px;width:130px">Kategori</th>
<th style="padding:12px 15px;width:60px;text-align:center">Urutan</th>
<th style="padding:12px 15px;width:90px;text-align:center">Status</th>
<th style="padding:12px 15px;width:130px;text-align:center">Aksi</th>
</tr></thead>
<tbody>
...
<?php if($imgs->num_rows > 0): ?><div class="fas-no-img" title="<?= $imgs->num_rows ?> gambar lagi" style="width:30px;height:30px;font-size:0.7rem;border-radius:8px">+<?= $imgs->num_rows ?></div><?php endif; ?>
<?php else: ?>
<div class="fas-no-img" style="width:45px;height:45px"><i class="fas fa-image"></i></div>
<?php endif; ?>
</div>
</td>
<td style="padding:12px 15px"><div style="font-weight:700;color:var(--dark)"><?= htmlspecialchars($f['nama']) ?></div>
...
<td style="padding:12px 15px"><span style="display:inline-flex;align-items:center;gap:4px;background:<?= htmlspecialchars($f['color']) ?>15;color:<?= htmlspecialchars($f['color']) ?>;padding:4px 10px;border-radius:8px;font-weight:600;font-size:0.75rem;white-space:nowrap"><span class="fas-color-dot" style="background:<?= htmlspecialchars($f['color']) ?>;width:12px;height:12px;border-width:1px"></span><?= htmlspecialchars($f['tag']) ?></span></td>
<td style="padding:12px 15px;text-align:center"><?= $f['sort_order'] ?></td>
<td style="padding:12px 15px;text-align:center"><span class="fas-status-badge <?= $f['is_active'] ? 'fas-status-active' : 'fas-status-inactive' ?>"><i class="fas fa-circle" style="font-size:0.4rem"></i><?= $f['is_active'] ? 'Aktif' : 'Nonaktif' ?></span></td>
<td style="padding:12px 15px;text-align:center"><div style="display:flex;gap:5px;justify-content:center">
<button class="vm-btn-action vm-btn-edit" onclick='openEditForm(<?= json_encode($f) ?>)' title="Edit" style="width:34px;height:34px;font-size:0.85rem"><i class="fas fa-edit"></i></button>
<a href="fasilitas-process.php?action=toggle_status&id=<?= $f['id'] ?>" class="vm-btn-action" style="background:rgba(14,165,233,0.1);color:#0ea5e9;width:34px;height:34px;font-size:0.85rem" title="Toggle Status"><i class="fas fa-power-off"></i></a>
<a href="javascript:void(0)" onclick="confirmDelete(<?= $f['id'] ?>,'<?= htmlspecialchars(addslashes($f['nama'])) ?>')" class="vm-btn-action vm-btn-delete" title="Hapus" style="width:34px;height:34px;font-size:0.85rem"><i class="fas fa-trash"></i></a>
</div></td>
</tr>
<?php endwhile; else: ?>
<tr><td colspan="7" style="text-align:center;padding:3rem;color:#94a3b8"><i class="fas fa-building" style="font-size:2.5rem;display:block;margin-bottom:10px;opacity:0.3"></i>Belum ada data fasilitas</td></tr>
<?php endif; ?>
</tbody></table></div>
</div>
</div>
